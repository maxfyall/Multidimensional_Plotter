...
*/
std::vector<std::string> ThreeDAxis::makeAxis(int boundaries, int bar)
{
	...
	glGenBuffers(1, &xAxisBufferObject);
	glBindBuffer(GL_ARRAY_BUFFER, xAxisBufferObject);
	glBufferData(GL_ARRAY_BUFFER, sizeof(xAxisVertex), xAxisVertex, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	/* Create Colour Buffer for X Axes */
	glGenBuffers(1, &xColourBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, xColourBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(xAxisColour), xAxisColour, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	/* Create Vertex Buffer for Y Axes */
	glGenBuffers(1, &yAxisBufferObject);
	glBindBuffer(GL_ARRAY_BUFFER, yAxisBufferObject);
	glBufferData(GL_ARRAY_BUFFER, sizeof(yAxisVertex), yAxisVertex, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	/* Create Colour Buffer for Y Axes */
	glGenBuffers(1, &yColourBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, yColourBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(yAxisColour), yAxisColour, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	/* Create Vertex Buffer for Z Axes */
	glGenBuffers(1, &zAxisBufferObject);
	glBindBuffer(GL_ARRAY_BUFFER, zAxisBufferObject);
	glBufferData(GL_ARRAY_BUFFER, sizeof(zAxisVertex), zAxisVertex, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	/* Create Colour Buffer for Z Axes */
	glGenBuffers(1, &zColourBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, zColourBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(zAxisColour), zAxisColour, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
